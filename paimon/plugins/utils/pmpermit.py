...
from paimon.utils.extras import reported_user_image
...
noPmMessage = bk_noPmMessage = (
    "▫️ `This is  A.I paimon Security` ▫️\n\n"
...
async def allow(message: Message):
    ...
    if userid:
        ...
        if a.matched_count:
            ...
            await (await paimon.get_users(userid)).unblock()
            await message.edit("`Aprovado para mensagem privada`", del_in=3)

        if userid in PMPERMIT_MSG:
            await paimon.delete_messages(userid, message_ids=PMPERMIT_MSG[userid])
...
@paimon.on_cmd(
    ...
    about={
        ...
        "paimon will not interfere or handle such private chats",
...
async def get_id(message: Message):
    ...
    if message.input_str:
        ...
        try:
            userid = (await paimon.get_users(user)).id
...
async def uninvitedPmHandler(message: Message):
    """pm message handler"""
    user_dict = await paimon.get_user_dict(message.from_user.id)
...
async def outgoing_auto_approve(message: Message):
    ...
    user_dict = await paimon.get_user_dict(userID)
