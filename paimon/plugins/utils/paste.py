...
class Pasting(PasteService):
    ...
    async def paste(
        ...
        async with ses.post(f'{self._url}api', json=data) as resp:
            return None if resp.status != 200 else self._url + (await resp.json())["key"]
...
class KatBin(PasteService):
    ...
    async def paste(
        ...
        if not token:
            return None
        async with ses.post(
                self._url, data={"_csrf_token": token, "paste[content]": text}
            ) as resp:
...
class SpaceBin(PasteService):
    ...
    async def paste(
        ...
        async with ses.post(
            self._api_url, data=dict(content=text, extension=ext)
        ) as resp:
            if resp.status != 201:
                return None
            code = _get_id(await resp.text())
...
async def paste_(message: Message) -> None:
    ...
    async with aiohttp.ClientSession(headers=_HEADERS) as ses:
        ...
        if url is None:
            await message.edit(
                f"`Failed to reach {service.get_name().title()}`", del_in=5
            )
        else:
            await message.edit(
    ...
    for service in _SERVICES.values():
        if service.is_supported(link):
            ...
            if code is None:
                ...
                return
            await message.edit(
                f"`Getting paste content [{service.get_name().title()}] ...`"
            )
            async with aiohttp.ClientSession(headers=_HEADERS) as ses:
