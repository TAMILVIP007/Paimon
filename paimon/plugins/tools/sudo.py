...
async def add_sudo(message: Message):
    ...
    if "-t" in message.flags:
        if user["id"] in Config.TRUSTED_SUDO_USERS:
            ...
            if (
                ...
                or user["id"] == (await paimon.get_me()).id
            ):
                await message.edit(
    ...
    if user["id"] in Config.SUDO_USERS:
        ...
        if user["id"] in Config.OWNER_ID or user["id"] == (await paimon.get_me()).id:
            await message.edit(
                f"user : `{user['id']}` is in `OWNER_ID` so no need to add in sudo",
                del_in=5,
            )
...
@paimon.on_cmd("vsudo", about={"header": "view sudo users"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if "-all" in message.flags:
        ...
        tmp_ = []
        for c_d in list(paimon.manager.enabled_commands):
    ...
    if cmd in Config.ALLOWED_COMMANDS:
        ...
        c_d.lstrip(Config.CMD_TRIGGER) for c_d in list(paimon.manager.enabled_commands)
    ):
...
@paimon.on_cmd("vscmd", about={"header": "view sudo cmds"}, allow_channels=False)
