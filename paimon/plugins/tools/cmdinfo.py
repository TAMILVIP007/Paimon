...
from paimon.utils import check_owner, humanbytes

plugin_regex = comp_regex(r"Path[\s:]{1,5}(paimon/plugins/\w+/\w+\.py)")
...
async def see_info(message: Message):
    ...
    cmd_list = list(paimon.manager.commands)
    found = True
    ...
    if plugin_loc == "/xtra":
        ...
        plugin_link = "{}/blob/{}/paimon/plugins{}/{}.py".format(
            Config.UPSTREAM_REPO, branch, plugin_loc, plugin_name
        )
    local_path = f"paimon/plugins{plugin_loc}/{plugin_name}.py"
...
if paimon.has_bot:
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^plugin_upload$"))
    @check_owner
    async def plugin_upload_(c_q: CallbackQuery):
        if match := plugin_regex.search(c_q.message.text):
            if os.path.exists(plugin_loc := match.group(1)):
                ...
                await paimon.bot.send_chat_action(
                    c_q.message.chat.id, "upload_document"
                )
                await paimon.bot.send_document(
