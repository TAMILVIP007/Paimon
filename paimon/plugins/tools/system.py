...
from paimon.utils import runcmd, terminate
...
@paimon.on_cmd(
    ...
    about={
        ...
        "flags": {
            "-h": "restart hard",
            "-log": "reload paimon-plugins repo (if 'restart -h' doesn't update custom plugins)",
...
async def restart_(message: Message):
    ...
    LOG.info("paimon-X Services - Restart initiated")
    ...
    if "h" in message.flags:
        if Config.HEROKU_APP:
            ...
            await FROZEN.drop()
            asyncio.get_event_loop().create_task(paimon.restart(hard=True))
    else:
...
@paimon.on_cmd(
    "shutdown", about={"header": "shutdown paimon-X :)"}, allow_channels=False
)
...
@paimon.on_cmd("listdisabled", about={"header": "List all disabled chats."})
...
@paimon.on_cmd(
    "sleep (\\d+)",
    about={"header": "sleep paimon-X :P", "usage": "{tr}sleep [timeout in seconds]"},
    allow_channels=False,
)
...
async def _slp_wrkr(seconds: int) -> None:
    ...
    await paimon.reload_plugins()
    await paimon.start()


@paimon.on_user_status()
