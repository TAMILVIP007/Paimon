...
class _GDrive:
    ...
    def _upload_file(self, file_path: str, parent_id: str) -> str:
        ...
        body = {
            "name": file_name,
            "mimeType": mime_type,
            "description": "Uploaded using paimon",
        }
        if parent_id:
...
@paimon.on_cmd("gsetup", about={"header": "Setup GDrive Creds"})
async def gsetup_(message: Message):
    ...
    if Config.G_DRIVE_CLIENT_ID and Config.G_DRIVE_CLIENT_SECRET:
        ...
        link = "https://fnixdev.gitbook.io/paimon/variaveis-necessarias-para-paimon/vars_opcionais/g_drive_client_id-and-g_drive_client_secret"
        await message.edit(
...
@paimon.on_cmd("gclear", about={"header": "Clear GDrive Creds"})
...
@paimon.on_cmd("greset", about={"header": "Reset parent id"})
...
@paimon.on_cmd("gempty", about={"header": "Empty the Trash"})
