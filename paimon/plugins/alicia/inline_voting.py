...
async def ivote_(message: Message):
    ...
    bot_u = (await paimon.bot.get_me()).username
    q = message.filtered_input_str
    query_ = f"anon_vote {q}" if "-a" in message.flags else f"voting {q}"
    res = await paimon.get_inline_bot_results(bot_u, query_)
    await gather(
        message.delete(),
        paimon.send_inline_bot_result(
...
@paimon.bot.on_callback_query(filters.regex(pattern=r"vote_.*"))
    ...
    try:
        ...
        if "up" in c_q.data:
            ...
            if tapper in votes_up:
                ...
                votes_up_names.remove((await paimon.get_users(tapper)).first_name)
                text_up -= 1
            else:
                votes_up.append(tapper)
                votes_up_names.append((await paimon.get_users(tapper)).first_name)
                text_up += 1
            ...
            if tapper in votes_down:
                ...
                votes_down_names.remove((await paimon.get_users(tapper)).first_name)
                text_down -= 1
            else:
                votes_down.append(tapper)
                votes_down_names.append((await paimon.get_users(tapper)).first_name)
                text_down += 1
            await VOTE.update_one(
                {"_id": f"vote_{id_}"},
                {"$set": {"down": votes_down}},
        ...
        await paimon.send_message(Config.LOG_CHANNEL_ID, f"```{tb}```")
