...
from paimon.utils import mention_html, time_formatter
from paimon.utils.extras import BotChat
...
if paimon.has_bot:
    ...
    async def bot_ban_(_, message: Message):
        ...
        start_ban = await paimon.bot.send_message(message.chat.id, "`Banindo")
        ...
        if not reason:
            ...
            return
        ban_user = await paimon.bot.get_user_dict(user_id, attr_dict=True)
    ...
    async def ban_from_bot_pm(ban_user, reason: str, log: str = False) -> None:
        user_ = await paimon.bot.get_user_dict(ban_user, attr_dict=True)
        banned_msg = (
        ...
        await asyncio.gather(
            ...
            paimon.bot.send_message(user_.id, banned_msg),
        )
        info = (
        ...
        if log:
            await paimon.getCLogger(log).log(info)
        return info
    ...
    async def uinfo_(_, message: Message):
        ...
        if uid_from_db := BOT_MSGS.search(reply.message_id):
            try:
                user_ = await paimon.bot.get_user_dict(uid_from_db, attr_dict=True)
            except Exception:
                pass
        elif user_from_fwd := reply.forward_from:
            user_ = await paimon.bot.get_user_dict(user_from_fwd, attr_dict=True)
