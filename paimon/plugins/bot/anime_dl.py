...
@paimon.on_cmd("gogo", about={"header": "Gogo Plugin Help"})
async def gogo_h(message: Message):
    if not paimon.has_bot:
        await message.err("You Need to create a bot via Bot Father", del_in=5)
        return
    link_ = "https://t.me/{}?start=inline".format((await paimon.bot.get_me()).username)
...
if paimon.has_bot:
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern="get_eps(.*)"))
    @check_owner
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"gogogetqual_(.*)"))
    @check_owner
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"get_currentpg(.*)"))
    @check_owner
