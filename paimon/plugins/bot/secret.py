...
from paimon import Config, paimon

if paimon.has_bot:

    @paimon.bot.on_callback_query(filters.regex(pattern=r"^(secret|troll)_(.*)"))
    async def alive_callback(_, c_q: CallbackQuery):
        secret_path = "paimon/xcache/secret.json"
        ...
        try:
            sender = await paimon.get_users(int(view_data.get("sender")))
        except (BadRequest, IndexError):
            sender = await paimon.get_me()
