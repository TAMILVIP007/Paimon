...
PATH = "paimon/xcache/emoji_data.txt"
CHANNEL = paimon.getCLogger(__name__)


if paimon.has_bot:

    @paimon.bot.on_callback_query(filters.regex(pattern=r"^op_(y|n)_(\d+)$"))
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^opresult_(\d+)$"))
