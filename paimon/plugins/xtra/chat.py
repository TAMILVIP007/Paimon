...
async def join_chat(message: Message):
    ...
    if not text:
        ...
        return
    try:
        chat = await paimon.get_chat(text)
        await paimon.join_chat(text)
        await paimon.send_message(text, f"```Joined {chat.title} Successfully...```")
...
@paimon.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}leave", "{tr}leave paimonOt"],
    },
    allow_private=False,
)
async def leave_chat(message: Message):
    ...
    try:
        await paimon.send_message(text, "```Good bye, Cruel World... :-) ```")
        await paimon.leave_chat(text)
    except UsernameNotOccupied:
        await message.edit(
...
async def invite_link(message: Message):
    ...
    if not user_id:
        try:
            chat = await paimon.get_chat(chat_id)
            chat_name = chat.title
            if chat.type in ["group", "supergroup"]:
                link = await paimon.export_chat_invite_link(chat_id)
                await message.edit(
        ...
        try:
            await paimon.add_chat_members(chat_id, user_id)
...
async def stagall_(message: Message):
    ...
    message_id = replied.message_id if replied else None
    member = paimon.iter_chat_members(chat_id)
    async for members in member:
        if not members.user.is_bot:
            text += mention_html(members.user.id, "\u200b")
    await message.delete()
    await paimon.send_message(chat_id, text, reply_to_message_id=message_id)
...
async def set_chat(message: Message):
    ...
    if "-title" in message.flags:
        await paimon.set_chat_title(message.chat.id, args.strip())
        await message.edit("```Chat Title is Successfully Updated...```", del_in=3)
    elif "-uname" in message.flags:
        try:
            await paimon.update_chat_username(message.chat.id, args.strip())
        except ValueError:
        ...
        try:
            await paimon.set_chat_description(message.chat.id, args.strip())
        except BadRequest:
...
async def bots_in_chat(message: Message):
    admin, member = [], []
    async for bots in paimon.iter_chat_members(message.chat.id, filter="bots"):
        status = bots.status
        u_id = bots.user.id
