...
from paimon.utils import progress
...
async def setname_(message: Message):
    ...
    if "-fname" in message.flags:
        ...
        if not lname:
            ...
            return
        await paimon.update_profile(first_name=fname.strip(), last_name=lname.strip())
        await message.edit(
...
async def poto_x(message: Message):
    ...
    if type_ in ("group", "supergroup", "channel") and message.client.is_bot:
        photo_ = await paimon.bot.download_media(f_id)
        await paimon.bot.send_photo(chat_id, photo_, reply_to_message_id=reply_id)
        os.remove(photo_)
    ...
    if "-p" in flags_:
        ...
        try:
            for poto_ in media_group:
                await paimon.send_media_group(chat_id, media=poto_)
