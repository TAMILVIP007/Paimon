...
@paimon.on_cmd("sysinfo", about="Obtenha informação da sua maquina virtual.")
async def get_sysinfo(message: Message):
    await message.edit("Obtendo informação do sistema...")
    response = await generate_sysinfo(paimon.workdir)
    await message.edit("<u>**Informações do Sistema**</u>:\n" + response)
