...
from paimon.utils import full_name
from paimon.utils import get_response as gr
...
async def f_stat(message: Message):
    ...
    try:
        get_u = await paimon.get_users(user_)
    ...
    bot_ = "MissRose_bot"
    try:
        query_ = await paimon.send_message(bot_, f"!fstat {user_id}")
...
async def fban_stat(message: Message):
    ...
    bot_ = "@missrose_bot"
    try:
        query_ = await paimon.send_message(bot_, f"!fbanstat {user_id} {fed_id}")
    except YouBlockedUser:
