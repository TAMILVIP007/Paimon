...
from paimon.utils import report_user
...
async def fban_(message: Message):
    ...
    async for data in FED_LIST.find():
        ...
        try:
            ...
            pass
        try:
            async with paimon.conversation(chat_id, timeout=8) as conv:
    ...
    await paimon.send_message(int(PROOF_CHANNEL), msg_)
...
async def fban_p(message: Message):
    ...
    if not FBAN_LOG_CHANNEL:
        ...
        return
    try:
        channel_ = await paimon.get_chat(int(FBAN_LOG_CHANNEL))
    except BaseException:
    ...
    if "-r" in message.flags:
        ...
        try:
            ...
            return
        try:
            msg_en = await paimon.get_messages(chat_id, int(msg_id))
    ...
    total = 0
    await message.edit(fban_arg[1])
    log_fwd = await paimon.forward_messages(
...
async def mass_fban(user, reason):
    async for data in FED_LIST.find():
        chat_id = int(data["chat_id"])
        try:
            await paimon.send_message(chat_id, f"/fban {user} {reason}")
