...
def get_thumb(result):
    ...
    open(os.path.join("./paimon/xcache/", thumb_name), "wb").write(thumb.content)
    return thumb_name


def down_song(link, filename):
    YouTube(link).streams.filter(only_audio=True)[0].download(
        "./paimon/xcache/", filename=filename
    )
...
"""
            try:
                await message.reply_audio(
                    audio=f"./paimon/xcache/{filename}",
                    caption=caption,
                    title=result[0]["title"],
                    thumb=f"./paimon/xcache/{thumb}",
                    duration=dur,
                )
            except Exception as e:
                ...
                await message.delete()
                os.remove(f"./paimon/xcache/{filename}")
                os.remove(f"./paimon/xcache/{thumb}")
