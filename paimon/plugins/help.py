...
from paimon.utils import get_file_id, get_response
from paimon.utils import parse_buttons as pb
...
if paimon.has_bot:
    ...
    async def callback_next_prev(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 1:
            buttons = parse_buttons(
                ...
                paimon.manager.get_all_plugins(),
            )
        elif len(pos_list) == 2:
            buttons = parse_buttons(
                p_num,
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"back\((.+)\)"))
    @check_owner
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"enter\((.+)\)"))
    @check_owner
    ...
    async def callback_manage(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 4:
            if is_filter(pos_list[-1]):
                flt = paimon.manager.filters[pos_list[-1]]
            else:
                flt = paimon.manager.commands[pos_list[-1]]
            await getattr(flt, task)()
            text, buttons = filter_data(cur_pos)
        else:
            plg = paimon.manager.plugins[pos_list[-1]]
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^mm$"))
    @check_owner
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^chgclnt$"))
    @check_owner
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"refresh\((.+)\)"))
    @check_owner
    ...
    async def inline_answer(_, inline_query: InlineQuery):
        ...
        if (
                    ...
                    for inline_content in inline_storage:
                        ...
                        if inline_db:
                            if (
                                ...
                                saved_msg = await paimon.bot.get_messages(
            ...
            if str_x[0].lower() in ["secret", "troll"] and len(str_x) == 3:
                ...
                msg = str_x[2]
                try:
                    receiver = await paimon.get_users(user_name)
