...
from paimon import Config, logbot, logging
from paimon.utils import time_formatter
from paimon.utils.exceptions import paimonBotNotFound
from paimon.plugins import get_all_plugins
...
class _Abstractpaimon(Methods, RawClient):
    @property
    ...
    async def load_plugin(self, name: str, reload_plugin: bool = False) -> None:
        """ Load plugin to paimon """
        _LOG.debug(_LOG_STR, f"Importing {name}")
        _IMPORTED.append(
            importlib.import_module(f"paimon.plugins.{name}"))
        if reload_plugin:
...
class paimonBot(_Abstractpaimon):
    ...
    @property
    def ubot(self) -> 'paimon':
        """ returns userbot """
        return self._bot


class paimon(_Abstractpaimon):
    """ paimon, the userbot """
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if Config.HU_STRING_SESSION and Config.BOT_TOKEN:
            ...
            kwargs['bot'] = paimonBot(bot=self, **kwargs)
    ...
    @property
    def bot(self) -> Union['paimonBot', 'paimon']:
        """ returns paimonbot """
        if self._bot is None:
            if Config.BOT_TOKEN:
                return self
            raise paimonBotNotFound("Need BOT_TOKEN ENV!")
        return self._bot
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        try:
            if coro:
                ...
                _LOG.info(_LOG_STR, f"paimon - {mode}")
                idle()
